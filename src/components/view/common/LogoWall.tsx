...
export function LogoWall({ className }: LogoWallProps) {
  return (
    <section className={cn('relative overflow-hidden bg-black/10 py-8 md:py-12', className)}>
                    ...
                    <div class="flex gap-8 md:gap-12 animate-scroll">
                      ${logos
                        ...
                        <div key="first-${index}" class="flex-shrink-0 flex items-center justify-center opacity-60 hover:opacity-100 transition-opacity">
                          <span class="text-xs md:text-sm text-foreground/40 font-medium">${logo.name}</span>
                        </div>
                      `
                        )
                        .join('')}
